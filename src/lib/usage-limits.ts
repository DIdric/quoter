...
/**
 * Get usage stats for display (uses user's own supabase client with RLS).
 */
export async function getUsageStats(
  supabase: ReturnType<typeof createClient>,
  userId: string,
  tier: SubscriptionTier = "free"
): Promise<{ quotesUsed: number; tokensUsed: number; costEstimate: number }> {
  const now = new Date();
  const monthStart = new Date(now.getFullYear(), now.getMonth(), 1).toISOString();
  ...
  const costEstimate = rows.reduce(
    (sum: number, r: { cost_estimate: number }) => sum + r.cost_estimate,
    0
  );

  return { quotesUsed: quotesUsed ?? 0, tokensUsed, costEstimate };
}
